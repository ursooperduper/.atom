(function() {
  ...
  module.exports = SymbolProvider = (function() {
    ...
    function SymbolProvider() {
      ...
      this.id = 'autocomplete-plus-symbolprovider';
    ...
    SymbolProvider.prototype.requestHandler = function(options) {
      var prefix, selection;
      if (options == null) {
        return;
      }
    ...
    SymbolProvider.prototype.findSuggestionsForWord = function(options) {
      ...
      words = atom.config.get("autocomplete-plus.strictMatching") ? symbolList.filter(function(match) {
        var _ref1;
        return ((_ref1 = match.word) != null ? _ref1.indexOf(options.prefix) : void 0) === 0;
    ...
    SymbolProvider.prototype.fuzzyFilter = function(symbolList, _arg) {
      var candidates, i, key, locality, position, prefix, results, rowDifference, score, symbol, wordsSeen, _i, _j, _len, _len1, _ref1;
      position = _arg.position, prefix = _arg.prefix;
      candidates = [];
      for (_i = 0, _len = symbolList.length; _i < _len; _i++) {
        symbol = symbolList[_i];
        if (prefix[0].toLowerCase() !== symbol.word[0].toLowerCase()) {
          continue;
        }
        score = fuzzaldrin.score(symbol.word, prefix);
        if (symbol.path === this.editor.getPath()) {
          score *= this.getLocalityScore(symbol, position);
        }
        if (score > 0) {
          candidates.push({
            symbol: symbol,
      ...
      for (i = _j = 0, _len1 = candidates.length; _j < _len1; i = ++_j) {
        ...
        key = this.getSymbolKey(symbol.word);
        if (!wordsSeen[key]) {
          results.push(symbol);
        }
        wordsSeen[key] = true;
    ...
    SymbolProvider.prototype.getLocalityScore = function(symbol, position) {
      ...
      if (symbol.bufferRows != null) {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          bufferRow = _ref1[_i];
          rowDifference = Math.min(rowDifference, bufferRow - position.row);
        }
    ...
    SymbolProvider.prototype.getSymbolsFromEditor = function(editor, minimumWordLength, tokenizedLines) {
      var bufferRow, cacheSymbol, key, matches, options, scopes, selector, symbol, symbols, token, tokens, type, word, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref1, _ref2, _results;
      if (tokenizedLines == null) {
        tokenizedLines = this.getTokenizedLines(editor);
      }
      symbols = {};
      cacheSymbol = (function(_this) {
        return function(word, type, bufferRow, scopes) {
          var cachedSymbol, cachedTypePriority, currentTypePriority, key;
          key = _this.getSymbolKey(word);
    ...
    SymbolProvider.prototype.getSymbolKey = function(word) {
      return word + '$$';
    };
